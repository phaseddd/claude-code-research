...
4. **花体/衬线大字体系** —— 它的大字是 sprite 贴图渲染（deferred text pass），不承担 webfont 成本；我们走 DOM 排版。中文 display 系其实不缺免费可商用选项（思源宋体 Heavy 900、OFL 书法系，Google Fonts unicode-range 分包实测单片 2.8~43KB），真正否决"花体贯穿"的是可读性（书法体只能大标题）与气质契合（数据/终端叙事）；主标题帅感走西文 display 图形化（`/insights` 命令本身做成大字图形），见 PLAN.md 决策记录
