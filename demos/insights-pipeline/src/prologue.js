...
export function mountPrologue({ uiEl, degraded = false, onEnter = null }) {
  ...
  root.innerHTML = `
    <h1 class="prologue-title">/<span class="hl">insights</span>：回车之后，发生了什么？</h1>
    <div class="prologue-body">
      <p>你输入 /insights，回车。屏幕卡住几十秒 —— 然后聊天窗弹出两行英文加一个 file:// 链接。点开，是一份漂亮的 HTML 使用报告：这个月你用 Claude 做了什么、哪里顺畅、哪里卡壳，甚至还有几条“要不要试试这个功能”的建议。</p>
      <p>这份报告不是凭空出现的。在你等待的那段时间里，一条看不见的流水线在工作：扫盘、缓存、压缩、打标签、并行写七章、合成总览。</p>
      <p>本演示带你走进那段等待时间 —— 一条数据河的旅程。</p>
    </div>
  ...
  // ---------- 3. 入场动画（总时长 ≈2.5s） ----------
  const bodyPs = root.querySelectorAll('.prologue-body p')
  ...
  if (reducedMotion) {
    ...
    entrance
      .to(titleChars, { opacity: 1, y: 0, filter: 'blur(0px)', duration: 0.45, stagger: 0.02 }) // 标题逐字（模糊锐化）
