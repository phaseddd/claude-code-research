...
> 状态：M0~M3 **实现落地**，演出化验收推进中。序章 3 秒版（三段声部布局 + WebGL 月球 + 按住进入 + **2026-08-12 转场重构验收通过**：星收环坍缩 → 月球布光 0.7s 白→青 → S1 面板落下）→ 第一幕三站实景：共享一条连续光河（终端命中 → 星云扫盘 → 缓存分流）+ 幕内轻量 gate（河全程全亮、相机贴河滑行）+ HUD 轨道。**当前节点：S1 开场与全部细节按序章标准逐轮打磨（主人控盘）**，随后 S2/S3 依次；第二/三幕仍为骨架占位（m4 重场戏规格推进中）。
>
> 2026-08-13 演出三件套（主人裁决）：序章流星（彗尾随路径生长、随机重投，动效偏好下不出场）+ 月球按转发条（按住加速自转 1.35×、松开回落）+ 河泛光层（河自持 RT + bloom 管线，主背景不动）。
...
体验流程：序章一次摆好（报告卡展品 + 点题句暗态；星空偶发流星掠过，2026-08-13）
→ 按住圆环 1.25s（模拟运行 /insights，520 颗星恒星坍缩收进环心，月球随按住加速自转
「上发条」，松手一并原速回退）→ 环满月球布光（0.7s 白→青扫屏，文本被光洗掉）
→ S1 面板落下（2026-08-12 转场重构验收版）。
...
河泛光层（2026-08-13）：`river.js` 自持渲染单元 —— 河独用场景渲染进 `EffectComposer.readBuffer`（RenderPass + UnrealBloomPass，线性域）→ 合成 quad 以自定义混合（颜色相加、alpha 不动）叠回主场景；主渲染器保持透明、CSS 渐变背景原样。`render/resize` 由引擎装配层按序调用。
