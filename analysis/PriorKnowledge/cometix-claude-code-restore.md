...
status: active
updated: 2026-07-15
applies_to: CometixSpace/claude-code master@213da58（镜像官方 v2.1.209）；官方 Claude Code v2.1.113+ Bun SEA 分发
tags:
  - topic:claude-code
  - topic:bun-sea
  - topic:npm
...
- 参考仓库最新提交：`213da58 changelog: sync v2.1.209`。
- 观察到的产物标签：`v2.1.193` … `v2.1.209`（含 `v2.1.196`–`v2.1.209` 连续标签）。
- 观察时间：2026-07。
...
- **`ripgrep` 不在 SEA 内嵌模块里**，是构建时另行下载放进 `vendor/` 的（易混淆点）。官方 SEA 里内置 `rg` 依赖 `Bun.isStandaloneExecutable`；恢复成 Node 包后该标志恒为 `false`，内嵌分支永不触发，因此 **vendor 里的 rg 必须经 polyfill 的 `Bun.which` 解析**（见 P6 配套模板），否则 Glob/Grep 会报 `ripgrep not found on PATH`。
...
**配套模板**：
- `templates/bun-polyfill.js`：在 Node 下挂 `globalThis.Bun`，并 patch `ws` 的 `{proxy}` → `{agent}`。覆盖的 API 面包括：
  - 基础：`hash` / `spawn` / `Terminal`（node-pty）/ `Transpiler` / `YAML` / `semver` / `which` / `stripANSI` / `stringWidth` / `wrapAnsi` / `gc` / `generateHeapSnapshot` / `embeddedFiles`
  - 运行时扩展（应对官方 2.1.200+ 对 Bun API 的硬依赖，例如 Windows bg-pty-host 的 `Bun.file`）：`file`（spawn stdio 经 `openSync` 转 fd——Node async spawn 不接受路径字符串作 stdio）、`deepEquals`、`listen`（TCP）、`serve`（HTTP(S)+fetch handler）、`stdin`、`WebView`（`{ closeAll() }` no-op）、`SQL`（构造即抛「gateway requires native binary」）
  - 标志位：`isStandaloneExecutable: false`（已从 SEA 抠出、按 Node 包跑；业务不应再走 multicall 内嵌路径）
  - `JSONL.parseChunk: null`（**故意 null**）：业务在 falsy 时走纯 JS 行解析 fallback；半吊子实现若只返回 `{ values, error }` 而缺 `{ read, done }`，会在 rewind 路径上 `while (undefined < r)` **死循环**
  - `which`：对 `rg` / `rg.exe` 先查 `__dirname/vendor/ripgrep/<arch>-<platform>/rg[.exe]`，再回退系统 `PATH`；尊重 `USE_BUILTIN_RIPGREP`（`unset/1/true` 优先 vendor，`0/false/no/off` 跳过 vendor）；系统查找用 `execFileSync(which|where, [cmd])` 避免 shell 注入
...
- `ripgrep` 来自 BurntSushi GitHub Release（默认 **`15.1.0`**）；`seccomp` 来自 `@anthropic-ai/sandbox-runtime`，**仅 Linux**。
- vendor 里有 rg 文件**不等于**搜索可用——业务通过 `Bun.which("rg")` 解析路径，见上节 polyfill。
...
- `publish`：npm **provenance**，命令用 **setup-node 自带的 `npm publish`**（不用 `npx npm@latest`——后者可能装出缺 `sigstore` 的坏树，导致 provenance `MODULE_NOT_FOUND`）；版本 > npm latest 用 `latest` tag，否则 `--tag backfill`；**先发平台包、再发主包**（主包 optionalDependencies 指向同版本平台包，顺序反了用户会装不到平台包）；已存在则跳过。
...
**相关但机制不同**：Node 模式下 `rg` 本体找不到，是 `isStandaloneExecutable===false` 导致内嵌 rg 不走、需 `Bun.which` 解析 vendor（见 §2 / §4 polyfill）；P5/P8 管的是 **bfs/ugrep shadow**，不是同一条路径。

**polyfill 半实现也危险**：返回「形状像但不完整」的 API（例如 `JSONL.parseChunk` 缺 `read`/`done`）会比 `null` 更糟——调用方走了 native 分支却没有完整语义，可能死循环。
...
8. **polyfill 宁缺毋滥**：业务有 pure-JS fallback 时，故意留 `null` 优于不完整的兼容实现。
...
- ❌「vendor 里有 rg 文件就等于搜索可用」→ 不够；Node 模式下靠 **`Bun.which` 的 vendor 查找**，否则 `isStandaloneExecutable===false` 时内嵌分支永不走。
- ❌「JSONL.parseChunk 应该完整 polyfill」→ 当前**故意 null**，让业务走纯 JS fallback；半吊子实现会无限循环。
...
- 关键 commit：`b19a743`（P5 反固化，2026-04-23）、`0bf75e5`（删 cron schedule，2026-04-23）、`65a72c4`（vendor rg via `Bun.which` + rg 15.1.0，#8）、`6c6f9fc`（CI bundled npm provenance）、`0212377` / `916a3c9`（polyfill：file/listen/serve… 与 `JSONL.parseChunk: null`）。
...
- 参考仓库处于活跃演进中，`master@213da58`（镜像官方 v2.1.209）之后的行为可能变化；本页按 `case` 记录**观察基线**，上游结构变动时应复核并可能转 `stale`。
