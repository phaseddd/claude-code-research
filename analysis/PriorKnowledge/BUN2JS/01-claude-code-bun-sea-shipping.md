...
第 3 步才对刚下的二进制调用 `extractBunSEA`。第 4 步才 `npm pack @anthropic-ai/claude-code@version`。抽出发生在 wrapper 下载之前，不是 wrapper 的后续步骤。工作链顺序见 [工作链](00-cometix-restore-pipeline.md)。
...
主包的 **cli.js** 来自 `templates/cli-placeholder.js`（[占位 cli.js](../../glossary.md)）：它只说明平台包没装上或 postinstall 被 `--ignore-scripts` 跳过，然后以退出码 1 结束。Cometix 既不用官方主包里的 JS 当入口，也不把抽出的脚本写进主包；抽出并打过补丁的入口在平台包里，安装时由 `install.cjs` 拷过来。组包细节见 [组 9 个平台包与主包并发布](05-cometix-npm-reassembly.md)。
