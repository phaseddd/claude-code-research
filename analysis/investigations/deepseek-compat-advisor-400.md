...
1. **不是 @cometix 恢复版的 bug**：注入逻辑（`dEo` 校验 / `ne.push` 注入 / 环境变量注册）在 2.1.219 cli.js 里完整且与官方文档描述一致；恢复版忠实镜像官方（补丁面见 [[04-node-compat-patches]]）。此项为**推断级**——官方自 v2.1.113 起无纯 JS 分发，无法逐字节对照官方原生二进制。
...
- [[00-cometix-restore-pipeline]] —— 本次故障所在客户端（恢复版）从官方 Bun SEA 还原成 Node npm 包的工作链
- [[04-node-compat-patches]] —— 该恢复版对抽出的 `cli.js` 做了哪些改写
