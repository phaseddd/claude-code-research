...
`/insights` 内嵌多套**固定英文提示词**：规定内部模型如何摘要会话、抽取会话标签、撰写报告各章，以及主会话模型如何向用户递交本地报告链接。本页给出 **2.1.209** 源码中的**英文原文、中文对照、请求拼装方式**；**不讲**斜杠命令如何分发（见机制页）。

## 配套机制

端到端流程：[Claude Code /insights 命令的端到端流程](../mechanisms/claude-code-insights-slash-command.md)。  
两页同一套 **P-id**。挂载点写在机制页对应步骤（L2 / L4.2(5)(6) / L5），不单开孤岛节：[端到端流程](../mechanisms/claude-code-insights-slash-command.md)。
...
**挂进上下文的方式**（挂载细节见机制页 L2 / L4.2(5)(6) / L5）：

| ID | 职责 | 挂到哪里 | 调用链 | tokens |
|---|---|---|---|---|
| **P-chunk-sum** | 超长投影分块摘要 | 内部 `hct` 的 **`userPrompt`** | `Osp` → `Km_` → `zm_` | **500** |
| **P-facet** | 单会话 facet 指南 | 同上 | `Osp` → `Zm_`（+ `Fm_`） | 与 schema 合计 **4096** |
| **P-facet-schema** | facet JSON schema | 与 P-facet 拼进同一 `userPrompt` | 同上 | 同上 |
| **P-section-*** | 报告七章 | 内部 `hct` 的 **`userPrompt`**（模板 + `DATA:`） | `Osp` → `th_` → `Isp` | 各章 **8192** |
| **P-at-a-glance** | 四段总览 | 同上 | `th_` 末尾 → 同一 `Isp` | **8192** |
| **P-user-reply** | 主会话强制分享话术 | **`Nsp` 字符串 → `Zxs` 的 `isMeta` message → `shouldQuery` 主会话** | `getPromptForCommand` 返回后 | 随主会话 |

`querySource:"insights"` 字面量 **3** 处：`zm_` / `Zm_` / `Isp`。P-user-reply **不在**这三处。运行时先并行七章再 glance；内部均为 `isNonInteractiveSession: true`。
...
`Osp` 把**未按 warmup 过滤**的完整 facet map 传给 `th_`（`Msp` 用过滤后集合）。纯 `warmup_minimal` 仍可能出现在采样列表。细节见机制页 L4。
